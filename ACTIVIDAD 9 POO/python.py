...
class DatosMeteorologicos:
    ...
    def procesar_datos(self) -> Tuple[float, float, float, float, str]:
        ...
        grados_viento = [self.DIRECCIONES_VIENTO[d] for d in dir_viento]
        dir_promedio = sum(grados_viento) / len(grados_viento)

        
        direccion_final = self.obtener_dir(dir_promedio)

        return (temp_promedio, humedad_promedio, presion_promedio, viento_promedio, direccion_final)

    def obtener_dir(self, grados: float) -> str:
